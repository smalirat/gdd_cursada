/* Escriba una consulta que retorne los pares de productos que hayan sido vendidos juntos (en la misma factura) más de 500 veces. 
...
SELECT 
    ...
    COUNT(*) AS VECES
FROM Factura F
JOIN Item_Factura I1 ON I1.item_tipo = F.fact_tipo
                     AND I1.item_sucursal = F.fact_sucursal
                     AND I1.item_numero = F.fact_numero
CROSS APPLY (
    SELECT I2.item_producto
...
JOIN Producto P2 ON P2.prod_codigo = Pares.item_producto
GROUP BY 
    P1.prod_codigo, P1.prod_detalle,
    P2.prod_codigo, P2.prod_detalle
HAVING COUNT(*) > 500
